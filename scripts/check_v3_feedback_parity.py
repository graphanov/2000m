#!/usr/bin/env python3
"""Check exact-diagnostics feedback parity for private v3 A/B run roots.

The checker is runner-layout aware and scorer-neutral: it does not run lanes,
change scoring semantics, or interpret outcomes. It only verifies that future
generation feedback packets can see exact scorer diagnostics/details rather than
aggregate AC IDs alone.
"""

from __future__ import annotations

import argparse
import json
import re
import sys
from pathlib import Path
from typing import Any

REQUIRED_RECORD_FILES = ("v3-result.json", "score.log", "scorer-feedback.md")
REQUIRED_WORKSPACE_FILES = REQUIRED_RECORD_FILES + ("feedback-manifest.json",)
DEFAULT_PILOT_SEEDS = [101, 202, 303]
DEFAULT_PILOT_LANES = ["A", "B"]


class FeedbackParityError(ValueError):
    pass


def load_json(path: Path) -> Any:
    try:
        return json.loads(path.read_text())
    except FileNotFoundError as exc:
        raise FeedbackParityError(f"missing JSON file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise FeedbackParityError(f"invalid JSON in {path}: {exc}") from exc


def lane_key(value: str) -> str:
    value = value.strip()
    if not value:
        raise argparse.ArgumentTypeError("lane id cannot be empty")
    return value.upper()


def lane_variants(lane: str) -> list[str]:
    variants: list[str] = []
    for value in (lane, lane.upper(), lane.lower()):
        if value and value not in variants:
            variants.append(value)
    return variants


def private_pair_dirs(run_root: Path) -> list[Path]:
    roots = [run_root, run_root / "private-pilot"]
    pair_dirs: dict[str, Path] = {}
    if run_root.is_dir() and re.fullmatch(r"pilot-seed-\d+", run_root.name):
        pair_dirs[run_root.resolve().as_posix()] = run_root
    for root in roots:
        for path in sorted(root.glob("pilot-seed-*")):
            if path.is_dir():
                pair_dirs[path.resolve().as_posix()] = path
        for path in sorted(root.glob("*/pilot-seed-*")):
            if path.is_dir():
                pair_dirs[path.resolve().as_posix()] = path
    return [pair_dirs[key] for key in sorted(pair_dirs)]


def discover_seeds(run_root: Path) -> list[int]:
    seeds: set[int] = set()
    records = run_root / "records"
    for path in sorted(records.glob("pilot-seed-*")):
        match = re.fullmatch(r"pilot-seed-(\d+)", path.name)
        if match:
            seeds.add(int(match.group(1)))
    for path in private_pair_dirs(run_root):
        match = re.fullmatch(r"pilot-seed-(\d+)", path.name)
        if match:
            seeds.add(int(match.group(1)))
    return sorted(seeds)


def discover_lanes(run_root: Path, seeds: list[int]) -> list[str]:
    lanes: set[str] = set()
    for seed in seeds:
        record_pair = run_root / "records" / f"pilot-seed-{seed}"
        pair_dirs = [record_pair] + [
            path for path in private_pair_dirs(run_root) if path.name == f"pilot-seed-{seed}"
        ]
        for pair in pair_dirs:
            for path in sorted(pair.glob("lane-*")):
                match = re.fullmatch(r"lane-([A-Za-z0-9_-]+)", path.name)
                if match:
                    lanes.add(match.group(1).upper())
    return sorted(lanes)


def first_existing_path(candidates: list[Path], fallback: Path) -> Path:
    for path in candidates:
        if path.exists():
            return path
    return fallback


def record_generation_dir(run_root: Path, seed: int, lane: str, generation: int) -> Path:
    candidates = [
        run_root / "records" / f"pilot-seed-{seed}" / f"lane-{variant}" / f"generation-{generation:02d}"
        for variant in lane_variants(lane)
    ]
    return first_existing_path(candidates, candidates[0])


def workspace_generation_dir(run_root: Path, seed: int, lane: str, generation: int) -> Path:
    candidates = [
        run_root
        / "workspaces"
        / f"2000m-v3-private-pilot-seed-{seed}-lane-{variant}"
        / "trajectory"
        / f"generation-{generation:02d}"
        for variant in lane_variants(lane)
    ]
    return first_existing_path(candidates, candidates[0])


def private_generation_dir(run_root: Path, seed: int, lane: str, generation: int) -> Path | None:
    pair_name = f"pilot-seed-{seed}"
    for pair in private_pair_dirs(run_root):
        if pair.name != pair_name:
            continue
        for variant in lane_variants(lane):
            candidate = pair / f"lane-{variant}" / f"generation-{generation:02d}"
            if candidate.exists():
                return candidate
    return None


def require_object(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise FeedbackParityError(f"{label} must be an object")
    return value


def scorer_diagnostic_lines(result_path: Path) -> list[str]:
    data = require_object(load_json(result_path), "v3 result")
    mechanical = require_object(data.get("mechanical"), "mechanical")
    failed_acs = mechanical.get("failedAcs", [])
    if not isinstance(failed_acs, list) or not all(isinstance(item, str) for item in failed_acs):
        raise FeedbackParityError(f"{result_path}: mechanical.failedAcs must be a string array")
    ranked = mechanical.get("ranked")
    failing_result = ranked is False or bool(failed_acs)
    acs = mechanical.get("acs")
    if failing_result and not isinstance(acs, list):
        raise FeedbackParityError(f"{result_path}: failing result is missing mechanical.acs exact diagnostics")

    lines: list[str] = []
    failed_items: list[dict[str, Any]] = []
    if isinstance(acs, list):
        for idx, raw_ac in enumerate(acs):
            ac = require_object(raw_ac, f"mechanical.acs[{idx}]")
            if ac.get("pass") is not False:
                continue
            ac_id = ac.get("id")
            name = ac.get("name")
            detail = ac.get("detail")
            if not isinstance(ac_id, str) or not ac_id.strip():
                raise FeedbackParityError(f"{result_path}: failed mechanical.acs[{idx}] has no id")
            if not isinstance(name, str) or not name.strip():
                raise FeedbackParityError(f"{result_path}: failed mechanical.acs[{idx}] has no name")
            if not isinstance(detail, str) or not detail.strip():
                raise FeedbackParityError(f"{result_path}: failed mechanical.acs {ac_id} has no exact detail")
            failed_items.append(ac)
            lines.append(f"{ac_id} {name}: {detail}")

        failed_item_ids = [ac["id"] for ac in failed_items]
        if failed_acs != failed_item_ids:
            raise FeedbackParityError(
                f"{result_path}: mechanical.failedAcs does not match failed mechanical.acs ids"
            )

    determinism = mechanical.get("determinism")
    if isinstance(determinism, dict) and determinism.get("pass") is False:
        details = determinism.get("details") or determinism.get("detail")
        if not isinstance(details, str) or not details.strip():
            raise FeedbackParityError(f"{result_path}: failing determinism has no exact details")
        lines.append(f"determinism: {details}")

    if failing_result and not lines:
        raise FeedbackParityError(f"{result_path}: failing result exposes no exact scorer diagnostic lines")

    seen: set[str] = set()
    unique: list[str] = []
    for line in lines:
        if line not in seen:
            seen.add(line)
            unique.append(line)
    return unique


def relative_or_str(path: Path, base: Path) -> str:
    try:
        return path.relative_to(base).as_posix()
    except ValueError:
        return str(path)


def same_file_content(left: Path, right: Path) -> bool:
    if not left.exists() or not right.exists():
        return False
    try:
        return left.read_bytes() == right.read_bytes()
    except OSError:
        return False


def check_generation(run_root: Path, seed: int, lane: str, generation: int, require_workspace_copy: bool) -> dict[str, Any]:
    lane_dir = record_generation_dir(run_root, seed, lane, generation)
    workspace_dir = workspace_generation_dir(run_root, seed, lane, generation)
    layout = "records-workspaces"
    if not lane_dir.exists() and not workspace_dir.exists():
        private_dir = private_generation_dir(run_root, seed, lane, generation)
        if private_dir is not None:
            layout = "private-pilot"
            lane_dir = private_dir
            workspace_dir = private_dir

    checks: dict[str, bool] = {}
    missing: list[str] = []
    errors: list[str] = []
    for filename in REQUIRED_RECORD_FILES:
        key = f"records/{filename}"
        exists = (lane_dir / filename).exists()
        checks[key] = exists
        if not exists:
            missing.append(relative_or_str(lane_dir / filename, run_root))

    if require_workspace_copy:
        for filename in REQUIRED_WORKSPACE_FILES:
            key = f"workspace/{filename}"
            exists = (workspace_dir / filename).exists()
            checks[key] = exists
            if not exists:
                missing.append(relative_or_str(workspace_dir / filename, run_root))
        if workspace_dir != lane_dir:
            for filename in REQUIRED_RECORD_FILES:
                record_path = lane_dir / filename
                workspace_path = workspace_dir / filename
                checks[f"workspace_matches_record/{filename}"] = same_file_content(record_path, workspace_path)

    feedback_path = workspace_dir / "scorer-feedback.md" if require_workspace_copy else lane_dir / "scorer-feedback.md"
    result_path = workspace_dir / "v3-result.json" if require_workspace_copy else lane_dir / "v3-result.json"
    feedback_text = feedback_path.read_text(errors="replace") if feedback_path.exists() else ""
    exact_section = "## Exact scorer diagnostics" in feedback_text
    diagnostic_lines: list[str] = []
    diagnostics_available = False
    diagnostic_visibility = False
    if result_path.exists():
        try:
            diagnostic_lines = scorer_diagnostic_lines(result_path)
            diagnostics_available = True
            diagnostic_visibility = (
                all(line in feedback_text for line in diagnostic_lines) if diagnostic_lines else exact_section
            )
        except FeedbackParityError as exc:
            errors.append(str(exc))

    checks["result_exact_diagnostics_available"] = diagnostics_available
    checks["feedback_has_exact_diagnostics_section"] = exact_section
    checks["feedback_contains_exact_diagnostics"] = diagnostic_visibility

    passed = all(checks.values()) and not errors
    return {
        "pairId": f"pilot-seed-{seed}",
        "taskSeed": seed,
        "lane": lane.upper(),
        "generation": generation,
        "layout": layout,
        "recordFeedbackDir": relative_or_str(lane_dir, run_root),
        "workspaceFeedbackDir": relative_or_str(workspace_dir, run_root),
        "checks": checks,
        "missing": missing,
        "errors": errors,
        "diagnosticCount": len(diagnostic_lines),
        "diagnosticLines": diagnostic_lines,
        "pass": passed,
    }


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check v3 private-pilot feedback parity before future generations use scorer feedback."
    )
    parser.add_argument(
        "run_root",
        help=(
            "Private v3 pilot run root containing records/ plus workspaces/, or "
            "private-pilot/<campaign>/<pair>/ lane directories."
        ),
    )
    parser.add_argument(
        "--seeds",
        nargs="*",
        type=int,
        help="Task seeds to check. Defaults to the frozen pilot seeds 101/202/303, or the seed in run_root when run_root is a pilot-seed-* pair directory.",
    )
    parser.add_argument("--lanes", nargs="*", type=lane_key, help="Lane IDs to check. Defaults to A/B.")
    parser.add_argument("--generations", nargs="*", type=int, help="Generation numbers to check. Defaults to 1..generation-cap.")
    parser.add_argument("--generation-cap", type=int, default=3, help="Generation cap used when --generations is omitted. Default: 3.")
    parser.add_argument("--records-only", action="store_true", help="Check generation records only. Default requires workspace-local feedback copies too.")
    parser.add_argument("--json-out", help="Write the full parity report JSON to this path.")
    args = parser.parse_args()

    run_root = Path(args.run_root).expanduser().resolve()
    if not run_root.exists():
        print(f"FAIL: run root does not exist: {run_root}", file=sys.stderr)
        return 1
    pair_root_match = re.fullmatch(r"pilot-seed-(\d+)", run_root.name)
    if args.seeds is not None:
        seeds = args.seeds
    elif pair_root_match:
        seeds = [int(pair_root_match.group(1))]
    else:
        seeds = DEFAULT_PILOT_SEEDS
    if not seeds:
        print("FAIL: no seeds supplied", file=sys.stderr)
        return 1
    lanes = args.lanes if args.lanes is not None else DEFAULT_PILOT_LANES
    if not lanes:
        print("FAIL: no lanes supplied", file=sys.stderr)
        return 1
    generations = args.generations or list(range(1, args.generation_cap + 1))
    if any(gen < 1 for gen in generations):
        print("FAIL: generation numbers must be >= 1", file=sys.stderr)
        return 1

    entries: list[dict[str, Any]] = []
    for generation in generations:
        for seed in seeds:
            for lane in lanes:
                entries.append(
                    check_generation(run_root, seed, lane, generation, require_workspace_copy=not args.records_only)
                )

    report = {
        "schemaVersion": "2000m.v3.feedback-parity-report.v1",
        "runRoot": str(run_root),
        "seeds": seeds,
        "lanes": [lane.upper() for lane in lanes],
        "generations": generations,
        "requiredRecordFiles": list(REQUIRED_RECORD_FILES),
        "requiredWorkspaceFiles": [] if args.records_only else list(REQUIRED_WORKSPACE_FILES),
        "pass": all(entry["pass"] for entry in entries),
        "entries": entries,
    }

    if args.json_out:
        out = Path(args.json_out).expanduser()
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps(report, indent=2) + "\n")

    if report["pass"]:
        print(f"OK: v3 feedback parity passed for {len(entries)} lane-generation records")
        return 0

    print("FAIL: v3 feedback parity failed", file=sys.stderr)
    for entry in entries:
        if not entry["pass"]:
            print(
                f"- {entry['pairId']} lane {entry['lane']} gen {entry['generation']}: "
                f"missing={entry['missing']} errors={entry['errors']} checks={entry['checks']}",
                file=sys.stderr,
            )
    return 1


if __name__ == "__main__":
    raise SystemExit(main())
